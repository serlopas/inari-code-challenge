...
class TestGuessRepositoryImpl:
    def test_create(self, guess_entity, session):
        ...
        guess_entity_returned = SqlAlchemyGuessRepositoryImpl(session).create(guess_entity)
        session.commit()
    ...
    def test_findall(self, session):
        ...
        guess_entities_returned = SqlAlchemyGuessRepositoryImpl(session).findall()
    ...
    def test_findall_but_table_empty(self, session):
        assert SqlAlchemyGuessRepositoryImpl(session).findall() == []

    def test_find_by_id(self, session):
        guess: Guess = GuessesFactory()

        guess_entity_returned = SqlAlchemyGuessRepositoryImpl(session).find_by_id(id=guess.id)
    ...
    def test_find_by_id_element_not_found(self, faker, session):
        assert SqlAlchemyGuessRepositoryImpl(session).find_by_id(id=faker.uuid4()) is None
    ...
    def test_update(self, faker, session):
        ...
        guess_entity_returned = SqlAlchemyGuessRepositoryImpl(session).update(entity=guess_entity)
    ...
    def test_delete_by_id(self, session):
        ...
        SqlAlchemyGuessRepositoryImpl(session).delete_by_id(id=guess.id)
    ...
    def test_find_by_game_id(self, session):
        ...
        guess_entity_returned = SqlAlchemyGuessRepositoryImpl(session).find_by_game_id(game_id=guess.game_id)
    ...
    def test_find_by_game_id_element_not_found(self, faker, session):
        assert SqlAlchemyGuessRepositoryImpl(session).find_by_game_id(game_id=faker.uuid4()) is None
